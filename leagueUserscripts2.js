                ...
                function actualScript() {
                    ...
                    function chat(chatMessage) {
                        ...
                        if (timeDiff > limit) {
                            ...
                            active = true;
                            setTimeout(function (chatMessage) {
                                chat(chatMessage);
				active = false;
			    }, limit - timeDiff, chatMessage);
                        }
                    }
                }
...
if ((window.sessionStorage.toggles && JSON.parse(window.sessionStorage.toggles).balltrans)){
tagpro.ready(function () {
    var tr = tagpro.renderer;
    var ballTransparency = 75;
    // Transparency
                ...
                function finalize(finished) {
                    ...
                    if (unfinalized && complete && gameStart == gameStart) {
                        ...
                        var submit =
                        {
                            ...
                            group: group,
                            date: gameNow.getTime() / 1000 >>> 0,
                            timeLimit: timeLimit,
                            duration: Math.max(((now - lastNow) * .06 >>> 0) + lastTime, minTime) - gameStart,
                            finished: finished,
                            map: {
                                name: mapInfo.name || 'Untitled',
                                author: mapInfo.author || 'Unknown',
                            ...
                            teams: [{
                                name: teamNames[0],
                                score: teamScores[0],
                                splats: btoa(splats[0].pad())
                            }, {name: teamNames[1], score: teamScores[1], splats: btoa(splats[1].pad())}]
                        };
                                    ...
                                    var concatenate = function () {
                                        ...
                                        for (var i = 0, j = new Uint32Array(3); ;) {
                                            ...
                                            if (type & 1) event = player.timeline[i];
                                                ...
                                                event = new Event(time, emptySize);
                                                event.write(0, emptySize);
                                            }
                ...
                tagpro.socket.on('p', function (data) {
                    ...
                    for (var i = 0; i < data.length; i++) {
                        ...
                        if (player) player.message++;
                            ...
                            {
                                firstTime: gameStart == gameStart ? time : 0,
                                lastTime: 0,
                                firstName: '',
                                lastName: '',
                                lastAuth: false,
                                ...
                                timeline: [],
                                toggles: [[], [], []],
                                message: gameStart == gameStart ? 0 : 1
                            };
                        }
                        if (newPlayer.name) {
                    ...
                    for (var team = 0; team < 2; team++) {
                        ...
                        if (expectedSplats[team]) {
                            ...
                            var splat;
                            while (splat = queuedSplats[team].shift()) {
